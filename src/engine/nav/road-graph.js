...
// live Google `ctx.ROUTE`, and externally fetched map roads in `ctx.osmRoadSegs` (legacy name:
// Mapbox first, Overpass fallback). Drives lane-keep assist, the
...
export function createRoadGraph(ctx) {
  ...
  const forOsmSegsNear = (x, z, fn) => {
    if (ctx._osmBoxes && ctx._osmBoxes.length) {
      for (const box of ctx._osmBoxes) {
        if (Math.hypot(x - box.x, z - box.z) > box.r * 1.35) continue;
        for (const s of box.segs) fn(s, box.source || 'osm');
      }
      return;
    }
    for (const s of ctx.osmRoadSegs) fn(s, 'osm');
  };
  ...
  function nearestRoadLocation(x, z, opts = {}) {
    ...
    if (includeOsm) forOsmSegsNear(x, z, (s, source) => tryAB(s[0][0], s[0][1], s[1][0], s[1][1], source));
  ...
  function roadTargetAhead(x, z, yaw, speed) {
    const eachSeg = nearHomeRoads(x, z)
      ? fn => { for (const s of ctx.roadSegs) fn(s); }
      : fn => forOsmSegsNear(x, z, fn);   // hood graph near home, the fetched OSM graph everywhere else
    ...
    eachSeg(s => {
      ...
      if (mx * mx + mz * mz > 900) return;           // only roads within ~30 m (stay on THIS road)
