...
// ---- ADDITIVE: stage the compressed, SEPARATED game-ready set ----------------------------
// scripts/build_level_game.mjs emits a mobile-friendly set under exports/<slug>/game/:
//   terrain/roads/buildings/trees/creek/fences/collision.glb + heightfield.bin + manifest.json
// (the ~21 MB dahill set vs the ~138 MB+ full master). Stage that verbatim into
// StreamingAssets/<slug>/ so DaHilgLevelRuntime can prefer it, while the single
// StreamingAssets/<slug>.glb master (staged above) stays as the fallback. Missing game set
// per-level is fine — the runtime falls back to the master automatically.
// The set is copied byte-for-byte (already meshopt+KTX2 compressed by build_level_game.mjs);
// no re-encode here. .meta sidecars are generated by Unity on import (StreamingAssets metas
// are git-ignored, like the existing <slug>.glb.meta files).
const GAME_SET_FILES = [
  'manifest.json', 'heightfield.bin',
  'terrain.glb', 'roads.glb', 'buildings.glb', 'trees.glb', 'creek.glb', 'fences.glb', 'collision.glb',
];
function stageGameSet(slug) {
  const gameDir = EXPORT(slug, 'game');
  const manifest = path.join(gameDir, 'manifest.json');
  if (!existsSync(manifest)) return null; // no game set for this level — runtime uses the master
  const destDir = path.join(STREAMING_DIR, slug);
  // Clear any stale set so a layer removed upstream doesn't linger (its .meta is regenerated).
  rmSync(destDir, { recursive: true, force: true });
  mkdirSync(destDir, { recursive: true });
  let copied = 0;
  let bytes = 0;
  for (const f of GAME_SET_FILES) {
    const src = path.join(gameDir, f);
    if (!existsSync(src)) continue; // optional layers (roads/trees/creek/fences) may be absent
...
for (const lv of STREAMED_LEVELS) {
  ...
  // Stage the separated game set alongside the master GLB (additive; runtime prefers it).
  const gameSet = stageGameSet(lv.slug);
  if (gameSet) { gameSetCount++; extras += ` +game(${gameSet.copied}f ${(gameSet.bytes / 1e6).toFixed(1)}MB)`; }
...
console.log(`StreamingAssets staged ${stagedCount}/${STREAMED_LEVELS.length} streamed level GLB(s) ` +
  `(+${gameSetCount} separated game set(s)) -> ${path.relative(ROOT, STREAMING_DIR)}`);
