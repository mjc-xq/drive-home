...
function addCreekArtAndShrubs() {
  if (creekW && creekW.length >= 2) {
    ...
    ribbonPart(offsetLine(creekW, creekWidth / 2 + 0.75), 1.35, 0.18, bankPos, bankIdx);
    ribbonPart(offsetLine(creekW, -(creekWidth / 2 + 0.75)), 1.35, 0.18, bankPos, bankIdx);
    let phase = 0;
    ...
    if (bankIdx.length) scene.add(mkMesh(bankPos, bankIdx, 0x756d58, 'Creek_Banks', { rough: 0.95 }));
    if (flowIdx.length) scene.add(mkMesh(flowPos, flowIdx, 0x9fd6f1, 'Creek_FlowLines', { rough: 0.4 }));
    if (rockIdx.length) scene.add(mkMesh(rockPos, rockIdx, 0x77786f, 'Creek_Rocks', { rough: 0.9 }));
    if (reedPos.length) scene.add(mkMesh(reedPos, null, 0x607a3d, 'Creek_Reeds', { rough: 0.9 }));
  }
  ...
  if (shrubIdx.length) scene.add(mkMesh(shrubPos, shrubIdx, 0x4d7437, 'Shrubs', { rough: 0.95 }));
}
